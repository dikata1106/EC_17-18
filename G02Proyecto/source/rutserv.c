...
// Este procedimiento inicializa la tabla de interrupciones para que el gestor
// de interrupciones sepa que rutina de atencion tiene que ejecutar cuando le
// llega una peticion de interrupcion. Ademas es aqui donde se configuran los
// registros de control de los perifericos.
...
void ProgramarRegistrosControl() {
    ...
    TIMER0_CNT = TIMER0_CNT | 0x00C1;

    // TIMER0_DAT
    //   Se utiliza para indicar a partir de que valor tiene que empezar a
    //   contar (latch)
    TIMER0_DAT = 0xE000;
}
